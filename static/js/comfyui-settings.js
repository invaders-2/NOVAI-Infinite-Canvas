...
function renderComfyInstances(){
    ...
    el.innerHTML = comfyInstances.map((addr, i) => `
        <div style="display:flex;align-items:center;gap:6px;padding:4px;border:0;border-radius:9px;background:var(--soft)">
            <span style="width:18px;text-align:center;font-size:10.5px;color:var(--faint);font-weight:800">${i + 1}</span>
            <input class="small-input" type="text" value="${escapeAttr(addr)}" placeholder="host:port" oninput="updateComfyInstance(${i}, this.value)" style="flex:1;height:28px;padding:0 8px;border:0;border-radius:6px;background:var(--panel);color:var(--text);font-size:12px;font-family:var(--font-mono)">
...
function renderEditor(){
    ...
    subEl.textContent = trf('comfy.nodeStats', {nodes:Object.keys(currentWorkflow).length, fields:currentConfig.fields.length}) + (isBuiltin ? ` · ${tr('comfy.builtin')}` : '');
                ...
                <div class="node-card-head" onclick="this.parentElement.classList.toggle('expanded')">
                    <div style="display:flex;align-items:center;gap:12px;min-width:0;flex:1">
                        ...
                        <div style="min-width:0">
                            ...
                            <div class="node-id">${escapeHtml(nodeSub(node))} · #${escapeHtml(nodeId)} · ${trf('comfy.configurableCount', {count:inputs.length})}</div>
                        </div>
                    </div>
                    <div style="display:flex;align-items:center;gap:10px;flex:0 0 auto">
                        <div class="node-stats">${exposedCount > 0 ? trf('comfy.exposedCount', {count:exposedCount}) : ''}</div>
    ...
    const nodesHtml = Object.entries(currentWorkflow).map(([id, node]) => {
        ...
        return `
            <g class="gnode cat-${cat} ${exposedClass}" data-node-id="${escapeAttr(id)}" transform="translate(${pos.x},${pos.y})" onclick="openNodePopup('${escapeAttr(id)}', this)">
                ...
                ${exposedCount > 0 ? `<text class="gbadge" x="${NODE_W - 8}" y="42" text-anchor="end">${trf('comfy.usedCount', {count:exposedCount})}</text>` : ''}
            </g>
        `;
    }).join('');
...
async function onSave(){
    ...
    for(const f of currentConfig.fields){
        if(!f.name || !f.name.trim()){
            alert(trf('comfy.saveMissingName', {field:f.input})); return;
        }
    }
...
async function onDelete(){
    ...
    if(!confirm(trf('comfy.deleteConfirm', {name: currentConfig.title || selectedName}))) return;
    try {
