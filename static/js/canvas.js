...
function canvasOriginalMediaUrl(url){ return window.NovaMedia ? NovaMedia.originalMediaUrl(url) : _localOriginalMediaUrl(url); }
function _localOriginalMediaUrl(url){
...
function canvasFileNameFromUrl(url){ return window.NovaMedia ? NovaMedia.fileNameFromUrl(url) : _localFileNameFromUrl(url); }
function _localFileNameFromUrl(url=''){
    try {
...
function canvasProxiedMediaUrl(url, name=''){ return window.NovaMedia ? NovaMedia.proxiedMediaUrl(url, name) : _localProxiedMediaUrl(url, name); }
function _localProxiedMediaUrl(url, name=''){
...
function canvasDisplayMediaUrl(url, name=''){ return window.NovaMedia ? NovaMedia.displayMediaUrl(url, name) : _localDisplayMediaUrl(url, name); }
function _localDisplayMediaUrl(url, name=''){
    const raw = canvasOriginalMediaUrl(url);
    return /^https?:\/\//i.test(raw) ? canvasProxiedMediaUrl(raw, name) : raw;
}
function canvasMediaPreviewUrl(url, size=512){ return window.NovaMedia ? NovaMedia.mediaPreviewUrl(url, size) : _localMediaPreviewUrl(url, size); }
function _localMediaPreviewUrl(url, size=512){
    const raw = canvasOriginalMediaUrl(url);
...
function canvasPreviewImgHtml(url, size=512, attrs=''){ return window.NovaMedia ? NovaMedia.previewImgHtml(url, size, attrs) : _localPreviewImgHtml(url, size, attrs); }
function _localPreviewImgHtml(url, size=512, attrs=''){
...
function loadCanvasOriginalImageDimensions(url){ return window.NovaMedia ? NovaMedia.loadImageDimensions(url) : _localLoadImageDimensions(url); }
function _localLoadImageDimensions(url){
...
function canvasVideoPreviewHtml(url, size=512, attrs=''){ return window.NovaMedia ? NovaMedia.videoPreviewHtml(url, size, attrs) : _localVideoPreviewHtml(url, size, attrs); }
function _localVideoPreviewHtml(url, size=512, attrs=''){
    const original = canvasOriginalMediaUrl(url);
    const preview = canvasMediaPreviewUrl(original, size);
...
function canvasVideoFallbackHtml(url, attrs=''){ return window.NovaMedia ? NovaMedia.videoFallbackHtml(url, attrs) : _localVideoFallbackHtml(url, attrs); }
...
function canvasActivateVideoPreview(img){
    ...
    if(!target) {
        ...
        if(fallback){
            ...
            // 不能只调 initSmartVideoControls——它只绑定事件，不创建 .svc-* 控件，
            // 裸 video 没有控制条 DOM → 无进度条/暂停/全屏（桌面端 WKWebView 复现）。
            const original = canvasOriginalMediaUrl(fallback.dataset.url || fallback.getAttribute('src') || '');
            if(original){
                const tpl = document.createElement('template');
                tpl.innerHTML = canvasVideoPlayerHtml(original, fallback.dataset.videoPlayerAttrs || '');
...
function applyTheme(theme){
    if(window.NovaUtils){ NovaUtils.applyTheme(theme); shell?.classList.toggle('theme-dark', theme === 'dark'); return; }
...
async function responseErrorMessage(response, fallback='请求失败'){
    if(window.NovaUtils) return NovaUtils.responseErrorMessage(response, fallback);
    // fallback implementation for backward compatibility
    try {
...
// Clipboard helpers: delegate to NovaUtils when available, keep local fallback
...
function copyTextWithTextarea(value){ return window.NovaUtils?.copyTextWithTextarea?.(value) ?? _localCopyTextWithTextarea(value); }
function _localCopyTextWithTextarea(value){
    let ta = null;
    try {
...
async function clipboardMatchesText(value){ return window.NovaUtils ? NovaUtils.clipboardMatchesText(value) : _localClipboardMatchesText(value); }
async function _localClipboardMatchesText(value){
    try {
...
async function copyTextToClipboard(text){ return window.NovaUtils ? NovaUtils.copyTextToClipboard(text) : _localCopyTextToClipboard(text); }
async function _localCopyTextToClipboard(text){
...
function startCanvasRemotePolling(){
    ...
    function poll(){
        remoteSyncInterval = setTimeout(async () => {
            await checkRemoteCanvasVersion();
            if(remoteSyncInterval) poll(); // 只在未被 stop 时继续
        }, 2500);
    }
    poll();
}
function stopCanvasRemotePolling(){
    if(remoteSyncInterval){
        clearTimeout(remoteSyncInterval);
...
async function urlToBase64(url){ if(window.NovaUtils) return NovaUtils.urlToBase64(url);
...
function renderNode(node){
    ...
    if(node.type === 'group') {
        ...
        const runnableCount = items.filter(n => canvasRunTypes().includes(n.type)).length;
        const parts = [];
...
function rhFieldKind(field){ if(window.NovaMedia) return NovaMedia.rhFieldKind(field);
...
function rhFieldRole(field){ if(window.NovaMedia) return NovaMedia.rhFieldRole(field);
...
function rhUsableFields(fields){ if(window.NovaMedia) return NovaMedia.rhUsableFields(fields);
...
async function runRhModelNode(node, opts={}){
    ...
    if(!opts.cascade){
        node.running = true;
        refreshRunNodes(node, out);
        /* node.running 在 finally 中清零，与实际 fetch 完成绑定 */
    }
    else refreshRunNodes(node, out);
    try {
        const taskInfos = await Promise.all(Array.from({length:count}, () => createCanvasImageTask(payload, {cascadeTargetId, runContext:opts.runContext})));
        if(!out){
            let outputs = [];
...
async function runComfyNode(nodeId, opts={}){
    ...
    const comfyTaskOptions = {cascadeTargetId, runContext:opts.runContext, taskResults:opts.taskResults};
    const sources = orderedSources(node, generatorSources(node));
    const prompt = sources.map(s => s.prompt).filter(Boolean).join('\n\n');
    const allRefs = sources.flatMap(s => s.refs || []);
...
async function runCanvasGroup(groupId){
    ...
    const runnable = [...memberIds].map(id => nodes.find(node => node.id === id)).filter(node => node && (canvasRunTypes().includes(node.type)));
    ...
    try {
        for(const layer of graph.layers){
            const settled = await Promise.allSettled(layer.map(async id => {
                ...
                try {
                    await runCascadeNodeByType(node, {cascade:true, groupId});
...
function formatRunDuration(ms){ return window.NovaUtils ? NovaUtils.formatRunDuration(ms) : _localFormatRunDuration(ms); }
function _localFormatRunDuration(ms){
    const total = Math.max(0, Math.round(Number(ms || 0) / 1000));
    const m = Math.floor(total / 60);
    const s = total % 60;
    return m ? `${m}m ${String(s).padStart(2, '0')}s` : `${s}s`;
}
function nowMs(){ return window.NovaUtils ? NovaUtils.nowMs() : Date.now(); }
...
function downloadBlob(blob, filename){ return window.NovaUtils ? NovaUtils.downloadBlob(blob, filename) : _localDownloadBlob(blob, filename); }
function _localDownloadBlob(blob, filename){
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = filename;
    document.body.appendChild(link);
...
// Mac触摸板手势处理：Safari 通过 gesturechange 缩放，Chrome 通过 wheel(ctrlKey) 缩放。
...
/* —— 页面卸载时清理定时器和周期任务，防止后台泄漏 —— */
window.addEventListener('beforeunload', () => {
    if(remoteSyncInterval){ clearTimeout(remoteSyncInterval); remoteSyncInterval = null; }
