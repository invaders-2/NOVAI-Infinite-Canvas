...
/** appId → 在途实例（singleton 并发去重安全网；本 Phase 同步执行，主要起防御作用） */
...
/** 为某实例打开（或重建）窗口 */
function openWindowFor(inst, def, source) {
  const adapter = getAdapter("iframe");
  const prepared = adapter.prepare(def);
  if (!prepared.ok) {
    store.update(inst.instanceId, { state: "error", error: prepared.error });
 ...
 * @returns {Object|null} AppInstance 或 null
 */
export function launch(appId, source = "user") {
  ...
  if (def.singleton) {
    ...
    if (existing) {
      ...
      if (existing.state === "background") {
        return openWindowFor(existing, def, source); // 复用同 instanceId 重建窗口
      }
      // launching/registered 中 → 正在启动，不重复开窗口
      return existing;
    }
 ...
 */
export function activate(instanceId, source = "user") {
  ...
  if (inst.windowIds.length > 0) {
    ...
    openWindowFor(inst, inst.def, source);
  }
  return inst;
}
...
/** stop + 重新 launch */
export function restart(instanceId, source = "user") {
  const inst = store.byInstanceId(instanceId);
  if (!inst) return null;
